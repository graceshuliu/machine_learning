...
ax.scatter(data_to_cluster['Market_val'],data_to_cluster['METERSZ'],data_to_cluster['BLUSAGECUBICFT'],c=kmeans.labels_.astype(float),s=50,alpha=0.5)
ax.scatter(centroids[:,0],centroids[:,4],centroids[:,1],c='red')
ax.set_xlabel('Market Value')
ax.set_ylabel('Meter Size')
